...
fn treb_init_creates_treb_directory_structure() {
    ...
    assert!(
        ...
        "registry.json should exist after init"
    );
    assert!(
...
fn run_with_env_delegates_correctly() {
    ...
    ctx.run_with_env(["version"], [("MY_TEST_VAR", "hello")])
        .success();
}
...
async fn anvil_node_reachable_via_context() {
    ...
    tokio::net::TcpStream::connect(&addr)
        .await
        .expect("anvil node should be reachable");
}
...
async fn anvil_port_rewrite_applied() {
    ...
    assert!(
        !content.contains("8545"),
        "foundry.toml should no longer contain port 8545"
    );
}
