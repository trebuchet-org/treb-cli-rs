...
fn compose_help_shows_all_flags() {
    let output = treb()
        .args(["compose", "--help"])
        .output()
        .expect("failed to run treb compose --help");
...
fn compose_dry_run_chain_shows_correct_order() {
    ...
    assert!(
        core_pos < periphery_pos,
        "core should come before periphery"
    );
}
...
fn compose_dry_run_diamond_shows_correct_order() {
    ...
    assert!(
        stderr.contains("1. base"),
        "base should be step 1: {}",
        stderr
    );
    assert!(
        stderr.contains("4. top"),
        "top should be step 4: {}",
        stderr
    );
}
...
fn compose_dry_run_json_is_valid() {
    ...
    for entry in arr {
        ...
        assert!(
            entry.get("component").is_some(),
            "entry should have component"
        );
...
fn compose_without_init_fails() {
    ...
    treb()
        ...
        .failure()
        .stderr(
            predicate::str::contains("treb init")
                .or(predicate::str::contains("foundry.toml")),
        );
}
