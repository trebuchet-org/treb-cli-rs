...
use framework::workdir::{DEFAULT_FIXTURE, TestWorkdir, port_rewrite_foundry_toml};
...
fn fixture_workdir_structure() {
    ...
    assert!(root.join("script/Deploy.s.sol").is_file(), "script/Deploy.s.sol missing");
    assert!(root.join("script/deploy/.gitkeep").is_file(), "script/deploy/.gitkeep missing");

    // Symlinked items (immutable).
    assert!(root.join("lib").exists(), "lib/ missing");
    assert!(root.join("lib").read_link().is_ok(), "lib/ should be a symlink");
    assert!(root.join("remappings.txt").exists(), "remappings.txt missing");
    assert!(root.join("remappings.txt").read_link().is_ok(), "remappings.txt should be a symlink");
    assert!(root.join(".gitignore").exists(), ".gitignore missing");
    assert!(root.join(".gitignore").read_link().is_ok(), ".gitignore should be a symlink");
    ...
    for name in &expected_sources {
        assert!(root.join("src").join(name).is_file(), "src/{name} missing");
    }
...
fn fixture_foundry_toml_port_rewrite() {
    ...
    assert!(before.contains("localhost:8545"), "original should have port 8545");
    assert!(before.contains("localhost:9545"), "original should have port 9545");

    // Rewrite both ports.
    port_rewrite_foundry_toml(root, &[(8545, 11111), (9545, 22222)]).unwrap();

    let after = fs::read_to_string(root.join("foundry.toml")).unwrap();
    assert!(after.contains("localhost:11111"), "port 8545 should be rewritten to 11111");
    assert!(after.contains("localhost:22222"), "port 9545 should be rewritten to 22222");
    assert!(!after.contains("localhost:8545"), "old port 8545 should be gone");
    assert!(!after.contains("localhost:9545"), "old port 9545 should be gone");
}
...
fn fixture_treb_toml_present() {
    ...
    assert!(content.contains("[accounts.anvil]"), "treb.toml should have [accounts.anvil]");
    assert!(
