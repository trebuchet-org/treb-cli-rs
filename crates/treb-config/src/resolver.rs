...
use crate::{
    convert_v1_to_resolved, detect_treb_config_format, extract_treb_senders_from_foundry,
    load_dotenv, load_local_config, load_treb_config_v1, load_treb_config_v2, ResolvedConfig,
    SenderConfig, TrebConfigFormat, TrebFileConfigV2,
};
...
pub fn resolve_config(opts: ResolveOpts) -> Result<ResolvedConfig> {
    ...
    if senders.is_empty() && format == TrebConfigFormat::None {
        let foundry_profile = if profile.is_empty() {
            "default"
        } else {
            &profile
        };
...
mod tests {
    ...
    fn make_v2_config() -> TrebFileConfigV2 {
        ...
        let mut default_ns = NamespaceRoles {
            profile: Some("default".to_string()),
            senders: HashMap::new(),
        };
        default_ns
            .senders
            .insert("deployer".to_string(), "deployer".to_string());
        config
            .namespace
            .insert("default".to_string(), default_ns);

        // production namespace: deployer -> ledger_signer, admin -> multisig.
        let mut production_ns = NamespaceRoles {
            profile: Some("optimized".to_string()),
            senders: HashMap::new(),
        };
        production_ns
            .senders
            .insert("deployer".to_string(), "ledger_signer".to_string());
        production_ns
            .senders
            .insert("admin".to_string(), "multisig".to_string());
        config
            .namespace
            .insert("production".to_string(), production_ns);
        ...
        ntt_ns
            .senders
            .insert("governance".to_string(), "multisig".to_string());
        config
            .namespace
            .insert("production.ntt".to_string(), ntt_ns);
    ...
    fn resolve_v2_three_level_deep() {
        let config = make_v2_config();
        let (profile, senders) =
            resolve_namespace_v2(&config, "production.ntt").unwrap();
        ...
        assert_eq!(senders["admin"].type_, Some(SenderType::Safe));
        assert_eq!(
            senders["governance"].type_,
            Some(SenderType::Safe)
        );
    }
    ...
    fn resolve_v2_role_to_account_mapping() {
        ...
        assert_eq!(
            ...
            Some("0xLedgerAddr".to_string())
        );
        assert_eq!(
            senders["deployer"].derivation_path,
    ...
    fn resolve_v2_missing_namespace_errors_with_available() {
        ...
        assert!(
            ...
            "error should mention requested namespace: {msg}"
        );
        assert!(
        ...
        assert!(
            ...
            "error should list available namespaces: {msg}"
        );
        assert!(
    ...
    fn resolve_v2_undefined_account_returns_default_sender() {
        ...
        let mut ns = NamespaceRoles {
            profile: None,
            senders: HashMap::new(),
        };
        ns.senders
            .insert("deployer".to_string(), "nonexistent_account".to_string());
    ...
    fn resolve_config_v2_with_local_config_and_env() {
        ...
        std::fs::write(
            tmp.path().join(".env"),
            "TREB_TEST_RESOLVE_KEY_US006=0xFromEnv\n",
        )
        .unwrap();
...
"#,
        ...
        assert_eq!(
            resolved.senders["deployer"].private_key,
            Some("0xFromEnv".to_string())
        );
        assert_eq!(
            resolved.fork_setup,
...
"#,
        ...
        assert_eq!(resolved.senders.len(), 1);
        assert_eq!(
            resolved.senders["deployer"].address.as_deref(),
            Some("0xFoundryDeployer")
        );
...
"#,
        ...
        assert_eq!(
            resolved.senders["extra"].type_,
            Some(SenderType::Ledger)
        );
        assert_eq!(
            resolved.senders["extra"].address.as_deref(),
            Some("0xCLIAddr")
        );
    }
