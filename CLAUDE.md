...
- **CLI help footers**: Contextual help-navigation footers in `crates/treb-cli/src/main.rs` belong on `after_long_help`, not `after_help`, so `treb <command> --help` shows the extra `Use "... [command] --help"` guidance while `-h` stays short
- **CLI flag help text sync**: When changing a subcommand flag description, update both the `#[arg(...)]` doc comment in `crates/treb-cli/src/main.rs` and the matching `Arg::help(...)` text in `crates/treb-cli/build.rs`, then refresh that command's `help_<command>` golden so clap help and generated completions stay aligned
...
- **CLI parse error formatting**: Go-style clap parse diagnostics are centralized in `crates/treb-cli/src/main.rs::custom_clap_error_message()`. If parser errors change, keep the human stderr path and the `--json` stderr wrapper aligned, and update both the unit assertions in `src/main.rs` and the binary-level checks in `crates/treb-cli/tests/cli_error_format.rs`
- **CLI parse exit codes**: Binary-level clap parse regressions in `crates/treb-cli/tests/cli_error_format.rs` should assert a non-zero exit code instead of hard-coding `1`; clap currently exits these parser failures with status `2` even when stderr is reformatted to the Go-compatible text
...
- **Config-backed namespace defaults**: Commands that accept `--namespace` should resolve the omitted case through `treb_config::resolve_config()` instead of hard-coding `"default"` so local config and namespace validation stay aligned with the rest of the CLI
...
- **Transaction routing model**: After script execution, `route_all()` in `crates/treb-forge/src/pipeline/routing.rs` partitions `BroadcastableTransactions` into runs by sender type, then dispatches: Wallet→broadcast, Safe→MultiSend+propose, Governor→build `Governor.propose()` calldata + recursively route through proposer. Governor routing is recursive (depth limit 4) — a Governor whose proposer is a Safe will produce a propose() tx that flows through Safe routing.
- **Governor+timelock broadcast address**: When a Governor sender has a timelock, `broadcast_address()` returns the timelock (not the governor) because the timelock is the on-chain executor (`msg.sender` when proposals execute). This affects `SENDER_CONFIGS` encoding, `partition_into_runs()` address matching, `sender_labels`, and `evm.sender`. `sender_address()` still returns the governor for display/identification.
- **Safe proposal polling**: After routing produces `SafeProposed` results, the CLI prompts interactive users to wait. `poll_safe_execution()` polls the Safe Transaction Service every 5s. Non-interactive mode always saves as Queued. The poll → update flow is in `crates/treb-cli/src/commands/run.rs::poll_proposed_results()`.
- **Routing result propagation**: `PipelineResult` carries `proposed_results: Vec<ProposedResult>` and `safe_transactions: Vec<SafeTransaction>` from routing. The orchestrator builds registry records from `RunResult::SafeProposed` / `GovernorProposed` and marks linked transactions as Queued.
